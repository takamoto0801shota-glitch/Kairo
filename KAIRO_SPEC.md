...
#### 🤝/📝 右側「具体的に」ボタン（補助表示）
- `🤝 今の状態について`（🟢/🟡）および `📝 今の状態について`（🔴）見出しの右側に、テキストリンク風の「具体的に」ボタンを表示してよい。
- `✅ 今すぐやること` 見出しの右側にも、同一デザイン・同一挙動の「具体的に」ボタンを表示する（スタイル差分を作らない）。

##### 🤝/📝 モーダルの目的
- 緊急度判定とは別に、「医学的に考えられる代表的な状態」を2つ提示する。
- ここでは **行動は一切書かない**。説明のみを表示する。

##### 🤝/📝 モーダルの表示仕様
- 疾患名を2つ表示する（最大2つ、**必ず2つ**）。
- 各疾患ごとに2〜3文で説明する。
- ユーザーの症状・入力内容は文章に含めない。
- 「あなたの場合」「この症状は」などの個別化表現は禁止。
- 診断を断定する表現は禁止（例：「〜です」→「〜とされる状態です」）。
- 合併症や重篤例の詳細説明は含めない。
- 恐怖を煽る表現は禁止。
...
- ユーザーの言葉を勝手に変換しない
- ❌「安静にしてください」→ ⭕「体が回復モードに入れる環境を作ります」
- ❌「水分を取りましょう」→ ⭕「脱水があると痛みが長引くため、少量ずつ補給します」

### 10.3 生成パイプライン（必須）
- `🤝/📝「具体的に」` の生成内容とスロット情報を元に、症状文脈を抽出する。
- 検索クエリは、主症状 + `🤝` 箇条書き全文 + `対処法` を連結して作る（例：`頭痛 痛みは9/10 吐き気がある きっかけは太陽光と睡眠不足 対処法`）。
- **検索の確実性**：日本語・英語それぞれに言語最適化クエリを用意し、全クエリを並列実行して結果を集約する。buildMandatoryGoogleQuery を優先し、複数クエリで重複除去・ランク付け。フォールバックは検索0件時のみ。
...
- 汎用フォールバック（「刺激を1つ減らし、水分を…」）は、部位が判別できない場合の最終手段のみ。
