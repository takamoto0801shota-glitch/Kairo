...
## 12.7 共感文の独立ルール（必須）
- 共感文は質問文に含めず、独立した短文として質問の前に表示する。
- セッション全体で必ず2回（初回共感は別枠）。
- 対象スロット：悪化傾向／経過時間／日常生活への影響／付随症状の質問直後。
- 同一セッション内で同じ共感文の再利用禁止。
- 共感文は「事実に対する感情の一言」に限定し、判断・安心保証は禁止。
- 共感の直後には必ず次の質問へ進む（共感で止まらない）。
- 共感文はAIが生成する（テンプレではない）。
