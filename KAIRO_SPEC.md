...
- 2件目以降はLLMまたはフォールバックで生成（曖昧表現禁止、番号付き禁止、医療行為の指示禁止）。
...
- **検索の確実性**：日本語・英語それぞれに言語最適化クエリを用意し、4段階のフォールバックで順次試行する（詳細→簡略→最小→汎用）。英語検索は主症状を英訳（頭痛→headache 等）して実行。
...
- 部位別の補足候補を常に用意し、`ensureActionCount` で必ず2件以上を確保する。
