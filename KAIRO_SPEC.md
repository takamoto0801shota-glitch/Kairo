...
#### 🤝 今の状態について（🟢/🟡のみ）
- 先に「判断に影響した重要要素のみ」を箇条書きで書く（全スロット列挙は禁止）
- 箇条書きは最大3〜4行
- 1行は短く、診療メモのように簡潔に書く
- 重複説明をしない
- 「〜に近い」の多用を禁止する
- 優先順位（この順で抽出）：
  1) 痛みスコア（必ず先頭）
  2) 以降はカテゴリ固定順ではなく、緊急度が高い順で抽出する
     - 候補要素：危険兆候 / 日常生活への影響 / 経過時間（数時間以上） / 付随症状（ある場合） / きっかけ（医学的に意味がある場合）
- ※この優先度ルールは固定（変更禁止）
...
- ただし `@kairo/KAIRO_SPEC.md:123-127` の RED抑制ガードが有効な場合のみ、1) 事実文は次の時間軸型に切り替える：
  - 「発症からの時間経過や症状推移からみて、急激な悪化を示す経過ではない」
...
- 目的：
  ...
  - 「今の状態について詳しく」由来の文脈 + 外部検索結果を統合し、状態に近い説明にする。
