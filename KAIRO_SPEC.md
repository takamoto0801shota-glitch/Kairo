...
### 10.1 🟡 / 🟢 の固定構造（必須）
- 既存の緊急度判定ロジック（Phase1/Phase2）は変更しない。
- 「✅ 今すぐやること」は毎回生成する（固定コピペ禁止）。
- daily_impact によって内容を分岐させる。

#### 🟡（中等度・緊急性なし）
- 構造：
  1) 冒頭サマリーを必ず2文（市販薬＋休息の合理性、回復優先）
  2) 具体的行動①（すぐできること＋理由）
  3) daily_impact 分岐
- daily_impact 分岐：
  - LIGHT / STRONG：休息推奨文を出す
  - NONE：OTC中心の文を出す
- MC表示：
  - LIGHT / STRONG のときのみ表示
  - NONE のときは表示しない
- MC表示文（固定）：
...
- 構造：
  ...
  2) 具体的行動②（セルフケア＋理由）
  3) daily_impact 分岐
- 「2️⃣ 刺激物は避けましょう / お酒やタバコは控えてください」の文言は使用禁止。
- daily_impact 分岐：
  - LIGHT：早めの休息を推奨
  - NONE：通常生活を継続しつつ経過観察
- MC表示：
...
#### 重要原則
- 「休息＝治療の一部」として表現する。
- 医学的妥当性を保つ（受診推奨ロジックは変更しない）。
- MCは機能低下フラグ（daily_impact）にのみ連動させる。
- 🟡は「回復モード」、🟢は「経過観察中心」のトーンを維持する。
- 「✅ 今すぐやること」は `・` 箇条書きで出す（1️⃣/2️⃣/3️⃣ を使わない）。
- 「✅ 今すぐやること」は「行動（・）」の直下に「理由（→）」を必ず1行で置く（理由に「・」を使わない）。
- 「✅ 今すぐやること」には、背中を押す断定文を含める（例：「今の症状レベルで無理を続けると、回復が遅れる可能性があります。今日は治す日にしてください。」）。
...
「具体的に教えてもらってもいいですか？」を追加する。  
7問目の回答後はまとめへ移行する。  
この内容はまとめの「🤝 今の状態について」で記載する。

ただし、原因カテゴリの質問に対してユーザーが自由記述で具体的な原因をすでに回答した場合は、
上記7問目を追加せず、そのまままとめへ移行する。
