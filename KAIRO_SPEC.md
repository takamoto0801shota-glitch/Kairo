...
- `🤝/📝` の「具体的に」は、ユーザー発話の言い換えを主目的にしない。
- 信頼ソース検索結果（状態説明 / 進行パターン / 見極めポイント）を主軸に、医学的に意味のある解釈として再構成する。
- ユーザー語の反復だけで本文を構成してはいけない。
- 押下時はモーダルで「状態パターンの整理」を表示する（診断・断定は禁止）。
- パターン生成の優先順位（固定）：
  1) きっかけ・思い当たること
  2) 症状特徴
  3) 強さ
...
- `きっかけ` が有効に取得できている場合：
  - パターンは **必ず2つ** 表示する。
  - 最初のパターンは必ず「きっかけベース」で構成する。
  - 2つ目は症状経過ベースで構成する。
  - どちらのパターン本文も **最大3文** とする。
  - 文中に必ずユーザー入力を反映し、「〜というきっかけがある場合」または「〜のあとに始まった痛みは」を含める。
- `特に思い当たらない` / `はっきりとは分からない` の場合のみ、従来の一般パターン生成を使う。
- 禁止：きっかけ入力の無視、汎用文への上書き、毎回同じ導入文の使い回し。
- モーダルの必須セクション表示条件：
  - `現時点の安心材料`（箇条書き）
  - `こんな変化があれば受診を検討`（最大3項目）
  - 上記2つは **🟢/🟡のときのみ表示** する。
- `🔴` のときは上記2セクションを表示しない。
- 🔴でも、`📝 今の状態について` の「具体的に」ボタン押下時は、上記と同じ優先順位・禁止事項でパターン生成する。
- 内部ロジックは「検索を裏側で利用」してよいが、UIは**説明生成UI**として固定する（検索画面・デバッグ画面にしない）。
- モーダル表示の固定構造（本番）：
  - `あなたの状態の理解を深める`（見出し固定）
  - `今の状態は、次のようなパターンと似ています。`
  - `■ パターン名` + 具体説明（簡潔に記載。常に2パターン、各パターン最大3文）
  - `現時点の安心材料`（🟢/🟡のみ）
...
- 禁止ワード（モーダル本文）：
  ...
  - 「可能性があります」の乱発
- デバッグ制御（実装固定）：
  - `const IS_DEBUG = false` を既定値にする。
  - `IS_DEBUG === true` のときのみ、抽出語・クエリ・ソート順位を表示可能。
...
## 10. 「✅ 今すぐやること」の生成ルール
- 目的：
  - ユーザーが「次に何をすればよいか」を迷わず実行できる具体行動を示す。
  - `🤝 今の状態について` の箇条書き情報（`@kairo/KAIRO_SPEC.md:337` / `@kairo/KAIRO_SPEC.md:343`）+ 主症状をクエリ化し、外部検索結果と統合して状態に近い説明にする。
...
- Google Custom Search を日本語・英語で実行する。
...
- OTCを無理に出さない（0件可）。
- `水分を取る / 横になる / 安静にする` は文脈一致が弱い場合は出力しない。
- 診断の断定表現は禁止。
- 単なる検索結果の羅列は禁止。
