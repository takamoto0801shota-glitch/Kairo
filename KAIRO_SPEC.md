...
- **conditional** = 条件付きで考慮。**本当の病名・疾患名**を記載する（例：群発頭痛、髄膜炎、脳腫瘍など）。症状の一般表現ではなく、医学的に正しい病名を使う。検索結果＋症状まとめから要約。固定テンプレート禁止。
...
- **🟡 状況によっては確認が必要**：2〜4件。**本当の病名・疾患名**を記載（例：群発頭痛、髄膜炎）。検索＋症状まとめから要約。煽らない表現。
- **🔴 すぐ受診が必要なサイン**：**2件のみ**（強制）。折りたたみ・初期非表示。「強い症状がある場合はこちら」で展開。
- `現時点の安心材料`（🟢/🟡のみ）
- `こんな変化があれば受診を検討`（🟢/🟡のみ、最大3）

##### 🤝/📝 出力例（ユーザーが「頭が痛い、昨日から、肩こりがある」と言った場合）
- **🟢**：2〜4件。**🟡**：2〜4件。**🔴**：**2件のみ**（強制）。
```
🟢 よくある原因（2〜4件）
・緊張型頭痛 → 昨日からの経過と肩こりが関係することがあります
・片頭痛 → 光や音に敏感になるタイプの頭痛です
・寝不足 → 睡眠不足で頭痛が出ることがあります
・脱水 → 水分不足で頭痛になることがあります
