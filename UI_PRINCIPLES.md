...
## 位置情報ステータスのUI（必須）
- 画面右上に常時表示する（ヘッダー内の固定表示）。
- usable の場合のみ表示する：
  - 📍 現在地を取得しました（location.status === "usable"）
- failed の場合は会話ログで一度だけ表示する：
  - 📍 現在地を確認できませんでした（一般的な案内になります）
- requesting は中間表示を出さない。
- 表示／非表示で他UIがズレてはいけない。
- 通常のステータス表示は会話ログの一部として扱ってはならない（failed の一度きり通知は例外）。
