...
- **同一の回答（同一メッセージ）内で、まとめの列（🟢〜🌱 または 🔴〜💬）を2回並べない。**「🟢／🟡／🔴 のここまでの情報を整理します」は**1回だけ**。**🤝 と 📝 の「Kairoの判断」は同じ回答に両方出さない**（🟢／🟡は 🤝 のみ、🔴は 📝 のみ）。
- **見出しの並び順は必ず守る（入れ替え・前後の逆転禁止）。**（B）🟢→🤝→✅→⏳→🌱、（A）🔴→📝→✅→🏥→💬。本文中に別ブロックを挟んでも、**見出し行の順番はこの通り**。
...
本文は**2文以内**。中心は**安心して休むことができる**こと（今の情報なら休んでよい・休むことへの許可が伝わる）。
（例：今の情報なら安心して休んで大丈夫です。まずは安静を優先してください。）＋必要なら（また不安になったら、ここで確認してください。）程度の1文まで
...
- 病院をおすすめする場合、「📝 Kairoの判断」から始めて、結論（病院をおすすめする）は必ず最後（🏥 受診先の候補）に出す
...
- ブロック構成は必ずフルセット
  ...
  - 病院推奨の場合：📝→✅→🏥→💬 の4ブロック
- 同一出力内でまとめ列を2回並べない。「ここまでの情報を整理します」は1回のみ。**🤝 と 📝 の「Kairoの判断」を併記しない**（🟢／🟡は 🤝 のみ、🔴は 📝 のみ）。
- 見出しの順番は厳守：様子見は 🟢→🤝→✅→⏳→🌱、病院は 🔴→📝→✅→🏥→💬（途中で順を入れ替えない）。
- 🟡は🟢と同じ構成で出力する
...
${requiredLevel === "🔴"
  ...
  - カテゴリ別：PAIN→痛みの相談、SKIN→皮膚の相談、GI→消化器の相談、INFECTION→発熱・喉の相談、TIRED→疲労・だるさ・肩こり・睡眠不足の相談など、症状に合わせて受診の肯定を具体的に。`
  : `- 🟢/🟡の場合：見出しは「🌱 最後に」。**2文以内**（「。」は2つまで）。短く簡潔に。
  - **中心軸**：本文は**安心して休むことができる**ことを核に組み立てる（今の情報なら休んでよい・落ち着いて休んでよいが伝わることを最優先。休むことへの不安を残さない）。
  - ①休息の指示を1文で（必ず「休息」を含める）。カテゴリに合わせて具体的に。上記の中心軸が伝わる表現を優先する：
    - PAIN（痛み系）：動きを控え、安静を優先する指示を。
    - SKIN（皮膚粘膜系）：刺激を避け、患部を休める指示を。
    - GI（消化器系）：消化を休める、食事を控える指示を。
    - INFECTION（発熱感染系）：休養・体を休める指示を。
    - TIRED（疲労・肩こり・睡眠不足系）：無理を減らし、睡眠と負荷のバランスを整える休息の指示を。
  - ②必要なら再訪や安心の一言を1文まで（①と重ならない範囲で。長い説明は禁止）。
  - トーン：優しいがはっきり指示。「〜してください」を優先。休むことへの許可と安心感が伝わるようにする。
  - 禁止：根拠のない「念のため」だけで休む不安を残す書き方、曖昧な表現。
...
function isHandshakeStateAboutHeaderLine(line) {
  return /^\s*🤝\s*(?:今の状態について|Kairoの判断)/.test(String(line || ""));
}

function textIncludesHandshakeAboutHeader(text) {
  return /🤝\s*(?:今の状態について|Kairoの判断)/.test(String(text || ""));
}

/** まとめ「🤝/📝 Kairoの判断」内の箇条書き（「・」）行の最低行数。これが無いと次ブロックに進まない。 */
const MIN_KAIRO_JUDGMENT_FACT_BULLETS = 2;

/**
 * まとめ本文中の Kairoの判断 ブロックに含まれる「・」行数を数える（🤝 / 📝 両方）。
 */
function countBulletLinesInKairoJudgmentBlock(text) {
  ...
  return body.filter((l) => /^\s*・/.test(String(l).trim())).length;
}

/**
 * state を更新し、Kairoの判断用の箇条書きを最低 minCount 行にする。組み合わせ行（〇〇＋〇〇）も箇条書き由来に揃える前提。
 * @returns {boolean} 目標行数に達したか
 */
...
function hasAllSummaryBlocks(text) {
  ...
  const required = isHospitalFlow(normalized)
    ...
    : normalized.includes("🟡")
      ? yellowHeaders
      : normalHeaders;
  const baseOk = required.every((header) => {
    if (header === "🔴 ここまでの情報を整理します") {
  ...
  if (
    isHospitalFlow(normalized) ||
    textIncludesMemoAboutHeader(normalized) ||
    textIncludesHandshakeAboutHeader(normalized)
  ) {
    if (countBulletLinesInKairoJudgmentBlock(normalized) < MIN_KAIRO_JUDGMENT_FACT_BULLETS) {
      return false;
    }
  }
  return true;
...
/** 次のまとめブロック見出し（本文中の改行区切り用） */
const SUMMARY_BLOCK_NEXT_HEADER_RE =
  /^(🟢|🟡|🔴|🤝|✅|⏳|🚨|💊|🌱|📝|⚠️|🏥|💬)\s/;

/**
 * 「🟢／🟡／🔴 のここまでの情報を整理します」が2回以上あるとき、**先頭のまとめ列のみ**残す（LLMがまとめ全文を二重出力したときの対策）。
 */
...
function removeSummaryBlockByHeaderPredicate(text, isHeaderLine) {
  const lines = String(text || "").split("\n");
  const start = lines.findIndex((l) => isHeaderLine(l));
  if (start < 0) return text;
  const nextIdx = lines.findIndex((l, i) => {
    if (i <= start) return false;
    return SUMMARY_BLOCK_NEXT_HEADER_RE.test(String(l || "").trimStart());
  });
...
/**
 * 緊急度に合わない「Kairoの判断」ブロック表現を除去：🔴 では 🤝 を、🟢／🟡 では 📝（メモ）を使わない。
 */
function stripWrongVariantStateAboutBlocks(text, level) {
  let t = String(text || "");
  if (level === "🔴") {
    while (textIncludesHandshakeAboutHeader(t)) {
      t = removeSummaryBlockByHeaderPredicate(t, isHandshakeStateAboutHeaderLine);
    }
  } else if (level === "🟢" || level === "🟡") {
    while (textIncludesMemoAboutHeader(t)) {
      t = removeSummaryBlockByHeaderPredicate(t, isMemoStateAboutHeaderLine);
    }
  }
  return t;
}
...
/**
 * まとめ本文を**仕様どおりの見出し順**に並べ替える（LLMが ✅→🤝 のように逆転させた場合の是正）。
 * 認識できたブロックだけを並べる（欠落があるときも、あるブロック同士の順序は正す）。
 * 先頭に見出し以外の行だけがある場合は、その前置きを可能な限り残す。
 */
function reorderSummaryBlocksToCanonicalOrder(text, level) {
  if (!text) return text;
  const normalizedText = preprocessSummaryDuplicateGuards(text, level);
  ...
  for (let i = 0; i < lines.length; i++) {
    if (headers.some((h) => lineMatchesSummaryHeaderFlex(lines[i], h))) {
      firstHeaderIdx = i;
      break;
    }
  }
  const preamble =
    firstHeaderIdx > 0 ? lines.slice(0, firstHeaderIdx).join("\n").trimEnd() : "";
  const fromFirstHeader = firstHeaderIdx < 0 ? cleaned : lines.slice(firstHeaderIdx).join("\n");
  const blocks = splitByKnownHeadersFirstWins(fromFirstHeader, headers);
  const ordered = headers.map((h) => blocks.get(h)).filter(Boolean);
  if (ordered.length === 0) return normalizedText;
  const body = ordered.join("\n\n").trim().replace(/\n{3,}/g, "\n\n");
  if (!preamble) return body;
  return `${preamble}\n\n${body}`;
}

/** 重複ブロック除去＋仕様順（欠落時も認識ブロック間の順序を正す） */
function dedupeSummaryBlocksByCanonicalOrder(text, level) {
  return reorderSummaryBlocksToCanonicalOrder(text, level);
}
...
async function enforceSummaryStructureStrict(text, level, history, state) {
  let normalizedText = preprocessSummaryDuplicateGuards(text, level);
  const headers = getRequiredSummaryHeadersByLevel(level);
  const cleaned = removeForbiddenSummaryBlocks(normalizedText, headers);
...
async function fetchPlaceDetails(placeId, { language = "en", includeOpeningHours = false } = {}) {
  ...
  if (String(placeId).startsWith("sg_curated_jp_")) return null;
...
function computeSingaporeCareScore(candidate) {
  ...
  const japaneseClinic = isJapaneseClinicForSingapore(candidate?.name) ? 3 : 0;
...
function symptomMatchScore(candidate, category) {
  ...
  } else if (category === "TIRED") {
    if (/内科|クリニック|internal|clinic|gp|general practitioner|family doctor/.test(hay)) score += 2;
  } else {
...
/**
 * 🇸🇬 シンガポール：日系クリニック判定（Places の表記ゆれ＋固定リスト）。
 * `japan|japanese` が無くても、下記キュレート施設名に合致すれば日系として扱う。
 */
function isJapaneseClinicForSingapore(name) {
  if (japaneseClinicNameHasJapanOrJapanese(name)) return true;
  const n = String(name || "").toLowerCase();
  if (/ことびあ|kotob\s*ia/i.test(String(name || ""))) return true;
  return SG_CURATED_JAPANESE_CLINICS.some((c) => n.includes(String(c.matchHint || "").toLowerCase()));
}

/**
 * シンガポール在住者向けに固定した日系クリニック（住所・座標は概算。距離ソート用）。
 * ユーザー位置に最も近い順で候補に合流させる。placeId は API 非依存表示用の合成 ID。
 */
const SG_CURATED_JAPANESE_CLINICS = [
  {
    id: "nihon-premium-novena",
    matchHint: "nihon premium",
    name: "Nihon Premium Clinic (ニホンプレミアムクリニック)",
    address: "10 Sinaran Dr, #11-12/13/30 Novena Medical Center, Singapore 307506",
    lat: 1.32058,
    lng: 103.84376,
  },
  {
    id: "japanese-association-adam",
    matchHint: "japanese association",
    name: "Japanese Association Clinic (日本人会クリニック)",
    address: "120 Adam Rd, Singapore 289899",
    lat: 1.32512,
    lng: 103.81523,
  },
  {
    id: "sugino-royal-square",
    matchHint: "sugino",
    name: "Sugino International Clinic (スギノインターナショナルクリニック)",
    address: "101 Irrawaddy Road, #10-11 Royal Square @ Novena, Singapore 329565",
    lat: 1.32189,
    lng: 103.84345,
  },
  {
    id: "japan-green-paragon",
    matchHint: "japan green",
    name: "Japan Green Clinic (ジャパングリーンクリニック)",
    address: "290 Orchard Road, #10-01 Paragon, Singapore 238859",
    lat: 1.30408,
    lng: 103.83565,
  },
  {
    id: "healthway-centrepoint",
    matchHint: "healthway japanese",
    name: "Healthway Japanese Medical (Centrepoint)",
    address: "176 Orchard Rd, #06-05 The Centrepoint, Singapore 238843",
    lat: 1.30045,
    lng: 103.83982,
  },
  {
    id: "raffles-japanese-shaw",
    matchHint: "raffles japanese",
    name: "Raffles Japanese Clinic (Shaw Centre)",
  ...
  },
  {
    id: "kotob-ue-square",
    matchHint: "kotob",
    name: "ことびあクリニック (UE Square)",
    address: "81 Clemenceau Ave, #03-11, Singapore 239917",
    lat: 1.29289,
    lng: 103.84012,
  },
  {
    id: "raffles-japanese-north-bridge",
    matchHint: "raffles japanese",
    name: "Raffles Japanese Clinic (North Bridge / 本院)",
    address: "585 North Bridge Road, Level 2, Singapore 188770",
    lat: 1.30123,
    lng: 103.8589,
  },
  {
    id: "raffles-japanese-great-world",
    matchHint: "great world",
    name: "Raffles Japanese Clinic (Great World)",
    address: "1 Kim Seng Promenade, #01-158B, Singapore 237994",
    lat: 1.29368,
    lng: 103.82746,
  },
  {
    id: "healthway-shenton",
    matchHint: "shenton way",
    name: "Healthway Japanese Medical (Downtown / Shenton)",
    address: "6A Shenton Way, #02-15, Singapore 068815",
    lat: 1.27894,
    lng: 103.85123,
  },
];
...
function buildSingaporeCuratedJapanesePlaces(origin) {
  if (!origin || !Number.isFinite(origin.lat) || !Number.isFinite(origin.lng)) return [];
  const out = SG_CURATED_JAPANESE_CLINICS.map((c) => {
    const distanceM = distanceMeters(origin.lat, origin.lng, c.lat, c.lng);
    const placeId = `sg_curated_jp_${c.id}`;
    return {
      name: c.name,
      placeId,
      vicinity: c.address,
...
async function fetchCarePlacesForSingapore(location, state) {
  ...
  for (const q of jpQueries) {
    ...
    const strictSoFar = mergePlaces(japaneseMerged).filter((c) => isJapaneseClinicForSingapore(c?.name));
    if (strictSoFar.length >= 3) break;
  }
  const curatedJp = buildSingaporeCuratedJapanesePlaces(location);
  const japanese = filterSingaporeExcluded(
    mergePlaces(curatedJp, mergePlaces(japaneseMerged)).filter((c) => isJapaneseClinicForSingapore(c?.name)),
    allowDental
  ).slice(0, 6);
  ...
  const gp = filterSingaporeExcluded(
    mergePlaces(gpRaw).filter((c) => !isJapaneseClinicForSingapore(c?.name)),
    allowDental
  ).filter((c) => (c?.distanceM ?? 0) <= 1000);

  // ③④ フィルタ（rating 3.8+）＋スコアリング（日系キュレートは rating 付与済みで通過）
  ...
  const gpSorted = meetsRating
    .filter((c) => !isJapaneseClinicForSingapore(c?.name))
    .sort((a, b) => (b._sgScore || 0) - (a._sgScore || 0));
  const japaneseSorted = meetsRating
    .filter((c) => isJapaneseClinicForSingapore(c?.name))
    .sort((a, b) => {
      const da = a?.distanceM ?? 999999;
      const db = b?.distanceM ?? 999999;
      if (da !== db) return da - db;
      return (b._sgScore || 0) - (a._sgScore || 0);
    });
...
async function fetchJapaneseClinicSingaporeForRedModal(state) {
  ...
  const curated = buildSingaporeCuratedJapanesePlaces(location);
  if (!getPlacesApiKey() || !location?.lat || !location?.lng) return curated.slice(0, 3);
  const queries = [
    "japanese clinic singapore",
  ...
  for (const q of queries) {
    ...
    const strict = mergePlaces(all).filter((c) => isJapaneseClinicForSingapore(c?.name));
    if (strict.length >= 3) break;
  }
  const merged = mergePlaces(curated, mergePlaces(all)).filter((c) => isJapaneseClinicForSingapore(c?.name));
...
function scoreSingaporePreference(candidate) {
  ...
  const hasJapaneseClinic = isJapaneseClinicForSingapore(candidate?.name);
...
function reorderSingaporePlacesWithJapaneseSecond(places) {
  ...
  const jpIdx = rest.findIndex((p) => isJapaneseClinicForSingapore(p?.name));
...
function buildSingaporeHospitalCandidatesThreeOrdered(placesIn) {
  ...
  const jp = uniq.filter((c) => isJapaneseClinicForSingapore(c?.name));
  const nonJp = uniq.filter((c) => !isJapaneseClinicForSingapore(c?.name));
  ...
  if (jpTwo) out.push(jpTwo);
  else if (secondGp) out.push(secondGp);
...
async function ensureSingaporeJapaneseClinicSecondSlot(state, placesIn) {
  ...
  const hasSecondJp = pool.length > 1 && isJapaneseClinicForSingapore(pool[1]?.name);
  if (hasSecondJp) return buildSingaporeHospitalCandidatesFourOrdered(pool);
  const hasAnyJp = pool.some((p) => isJapaneseClinicForSingapore(p?.name));
...
function getInsuranceLabel(facility, isSingapore) {
  ...
  if (isJapaneseClinicForSingapore(facility?.name) || /(日系|nihon)/i.test(text)) return "［保険: △ 要確認（自費になることも）］";
    ...
    if (isSingapore) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (i > 0) {
          ...
          if (i === 1) {
            lines.push(
              isJapaneseClinicForSingapore(c?.name)
...
async function buildSingaporeModalContent(places, plan, state) {
  ...
  if (!pool.some((p) => isJapaneseClinicForSingapore(p?.name))) {
    const extra = await fetchJapaneseClinicSingaporeForRedModal(state);
  ...
  const jpSlots = pool.slice(1, 3).filter((p) => p && isJapaneseClinicForSingapore(p.name));
  let jpIdxShown = 0;
  for (const jpSlot of jpSlots) {
    if (!jpSlot || jpSlot.placeId === main.placeId) continue;
  ...
  const auxGps = pool.slice(1).filter((p) => p?.placeId && !isJapaneseClinicForSingapore(p?.name) && p.placeId !== main.placeId);
  const thirdGp = auxGps[0] || null;
...
async function replaceStateAboutBlockOnly(summaryText, state, historyText = "") {
  ...
  if (hasRedStateBlock) {
    ...
    result = replaceSummaryBlock(result, "📝 Kairoの判断", memoLines);
    return result;
  }
  const level = finalizeRiskLevel(state);
  ...
  const newBlock = ["🤝 Kairoの判断", body].join("\n");
  return replaceSummaryBlock(summaryText, "🤝 Kairoの判断", newBlock);
}
...
async function ensureOutlookBlockAsync(text, state) {
  let block = null;
  try {
    block = await buildOutlookBlockWithLlm(state);
  } catch (_) {
    /* 静的へ */
  }
...
/**
 * PAIN/INFECTION 時のパナドール（シンガポール市販）: 青・赤・緑のいずれか1つ。
 * - 赤 Extra: 痛みスコアが 8 を超える（9〜10）
 * - 緑 Cold & Flu: 風の初期症状パターン（KAIRO_SPEC）または喉主症状
 * - 青オリジナル: 上記以外の基本
 */
...
function resolvePanadolPackForPainInfection(state, topic) {
  ...
  if (pain != null && pain > 8) return "red";
  if (topic === "喉") return "green";
  if (shouldUseWindyColdOnsetPatternForStateAbout(state)) return "green";
  return "blue";
}
...
function applyPanadolSingaporeDisplayToDoActionItems(items, state, ctx) {
  ...
  return items.map((x) => {
    ...
    const z = ensurePanadolSingaporeDisplayCopy(action, x.reason, state, topic);
    if (x.title !== undefined) return { title: z.action, reason: z.reason };
    return { action: z.action, reason: z.reason };
  });
}

function getPanadolOtcActionForModal(state, topic) {
  ...
  return byPack[pack] || byPack.blue;
}

function getOtcActionForYellowModal(state, topic) {
  const normalizedTopic =
    typeof topic === "string" && topic.trim() ? normalizeContextLocation(topic) : normalizeContextLocation(topic || "");
  const cat = state ? state.triageCategory || resolveQuestionCategoryFromState(state) : null;
  const usePanadol = state && (cat === "PAIN" || cat === "INFECTION");

  if (usePanadol && (normalizedTopic === "頭" || normalizedTopic === "喉")) {
    return getPanadolOtcActionForModal(state, normalizedTopic);
  }
...
async function refineDoActionsWithLLM(plan, state, level, options = {}) {
  ...
  const minRequired = forSummary ? (summaryPainInfectionYellow ? 1 : 2) : 4;
...
/**
 * 今すぐやることモーダル「■やらないほうがいいこと」：静的 seed を起点に、主症状・箇条書き・ evidence に即した2件を LLM で生成
 */
async function refineDontActionsWithLLM(plan, state, seedDontActions, actionSection = "") {
  const ctx = plan?.currentStateContext || buildCurrentStateContext(state, state?.historyTextForCare || "", String(actionSection || ""));
  const category = state?.triageCategory || resolveQuestionCategoryFromState(state);
  const level = state?.decisionLevel || "🟢";
  const mainSym = String(ctx?.mainSymptomDisplay || ctx?.mainSymptom || "症状").trim();
  const bulletHints = (buildStateFactsBullets(state, { forSummary: true }) || []).slice(0, 10).join("\n");

  for (let attempt = 0; attempt < LLM_RETRY_COUNT; attempt++) {
    try {
      const systemPrompt = [
        "一般向けセルフケアで、今の状態では避けた方がよい行動（やらないほうがいいこと）を2件。",
        "出力はJSONのみ: {\"dont\":[{\"action\":\"20字以内の短い行動名\",\"reason\":\"1文（40～90字目安）\"}]}",
        "action: 体言止め可。「〜は控える」「〜を避ける」など。同じテンプレの重複（例: 汎用の活動量だけ）禁止。",
        `主症状: 「${mainSym}」。緊急度: ${level}。カテゴリ: ${category}。`,
        "主症状の部位・性質（痛み/熱/のど/腹/皮膚/疲れ等）に即した2件にし、**どの症状でも同じ文に見える内容は禁止**",
        "ユーザーが言っていない病名の断定、恐怖、医療行為の禁止は厳守。",
        "検索要約（danger / observe / selfCare）に触れうる事があれば、忌避行動の理由に反映。",
        bulletHints ? `【今の整理（参考）】\n${bulletHints}` : "",
      ]
        .filter(Boolean)
        .join("\n");
      ...
      const userPayload = {
        seedDont: (Array.isArray(seedDontActions) ? seedDontActions : []).slice(0, 2),
        evidence: {
          danger: (plan?.evidence?.danger || []).slice(0, 6),
          observe: (plan?.evidence?.observe || []).slice(0, 6),
          selfCare: (plan?.evidence?.selfCare || []).slice(0, 4),
        },
        actionSectionExcerpt: String(actionSection || "").slice(0, 2000),
      };
      ...
      const parsed = parseJsonObjectFromText(completion?.choices?.[0]?.message?.content || "");
      const out = Array.isArray(parsed?.dont) ? parsed.dont : [];
      const valid = out
        .filter((d) => d && d.action && d.reason)
        .map((d) => ({
          action: toConciseActionTitle(String(d.action).trim()) || String(d.action).trim(),
          reason: String(ensureReliableReason(String(d.reason).trim(), plan?.evidence || {})).trim(),
        }))
        .filter((d) => d.action && d.reason);
      if (valid.length >= 2) return valid.slice(0, 2);
    } catch (_) {
      /* retry */
    }
  }
  return (Array.isArray(seedDontActions) ? seedDontActions : []).filter((d) => d && d.action && d.reason).slice(0, 2);
}
...
function buildDoActionsFromPlan(plan, state, level, options = {}) {
  ...
  /** 本文：常に2箇条書き。特例（PAIN/INFECTION+🟡）は固定1＋生成1のため、生成側の最低件数は1 */
  ...
  ensured = applyPanadolSingaporeDisplayToDoActionItems(ensured, state, ctx).map((x) => ({
...
async function generateLastBlockBodyWithLLM(level, state, contextText = "") {
  ...
  const categoryHint = (() => {
    if (level === "🔴") return "";
    const hints = {
      PAIN: "痛み系（頭痛・腰痛など）：動きを控え、安静を優先する指示を。**今の情報なら安心して休んでよい**ことが伝わるように。",
      SKIN: "皮膚粘膜系：刺激を避け、患部を休める指示を。**安心して休める**ことが中心になるように。",
      GI: "消化器系（腹痛・下痢など）：消化を休める、食事を控える指示を。**落ち着いて休んでよい**ことが伝わるように。",
      INFECTION: "発熱感染系（喉・咳・熱など）：休養・体を休める指示を。**休むことへの許可と安心**が伝わるように。",
      TIRED: "疲労・だるさ・肩こり・首こり・睡眠不足系：無理を減らし、休息とリズムを整える指示を。**安心して休める**ことを核に。",
    };
...
症状カテゴリ（${category}）と主症状（${mainSymptom || "症状"}）に合わせて、受診の肯定・後押しを具体的に書く。`
      : `見出しは「🌱 最後に」。**2文以内**（「。」は2つまで）。短く簡潔に。
**中心軸**：**安心して休むことができる**こと。今の情報なら休んで問題ない・落ち着いて休んでよいが伝わることを最優先。休むことへの不安を残さない。
①休息の指示を1文（必ず「休息」を含める。カテゴリに合わせて具体化：${categoryHint}）
②必要なら再訪や安心の一言を1文まで（①と重ならない範囲で。「また不安になったらここで確認」程度でよい。長い説明は禁止）
「〜してください」を優先。休むことへの許可と安心感が伝わるように。抽象的な励まし禁止。根拠のない「念のため」だけで休む不安を残すのは禁止。
  ...
  for (let i = 0; i < 10; i++) {
    try {
      const retry = await openai.chat.completions.create({
        model: "gpt-4o-mini",
        messages: [{ role: "user", content: level === "🔴" ? "「今の状況で受診を選ぶのは適切な判断です。」をそのまま返して。" : "「今の情報なら安心して休んで大丈夫です。まずは休息を優先してください。」をそのまま返して。" }],
        temperature: 0,
        max_tokens: 50,
      });
...
/**
 * `context.location` が PAIN/INFECTION 等のカテゴリ名になりがちで normalize が頭/腹/喉に当たらないため、
 * mainSymptom・表示ラベル・症状語から推論する（「やらないこと」が常に汎用テンプレに落ちるのを防ぐ）
 */
function inferDontActionTopicFromContext(context = {}) {
  const bundle = [
    context?.location,
    context?.mainSymptom,
    context?.mainSymptomDisplay,
    Array.isArray(context?.symptoms) ? context.symptoms.join(" ") : "",
    typeof context?.summaryFacts === "string" ? context.summaryFacts : Array.isArray(context?.summaryFacts) ? context.summaryFacts.join(" ") : "",
  ]
    .filter(Boolean)
    .join(" ")
    .replace(/\s+/g, "");
  if (!bundle) return "";
  if (/(腹|胃|腸|下痢|便秘|便|お腹|みぞおち|吐き気|嘔吐)/.test(bundle)) return "お腹";
  if (/(頭|こめかみ|片頭|後頭|ずきん|脳|ズキ|ずき|頭痛)/.test(bundle)) return "頭";
  if (/(喉|のど|咽|扁桃|上気道|咳|せき|痰|鼻水|鼻づまり|風邪|かぜ)/.test(bundle)) return "喉";
  if (/(皮膚|唇|かゆ|発疹|水ぶくれ|湿疹|蕁麻|紅潮)/.test(bundle)) return "皮膚";
  if (/(肩|首|眠|疲労|だる|倦怠|寝不足)/.test(bundle)) return "疲労";
  return "";
}

function buildDontActionsFromContext(context = {}, evidence = {}) {
  const topic =
    inferDontActionTopicFromContext(context) || normalizeContextLocation(context?.location || "");
  const base = [];
  if (topic === "頭") {
    base.push({
      action: "画面を長時間見続ける",
    ...
    base.push({
      action: "患部を掻きむしる・強くこする",
      reason: "刺激が重なると、局所の状態変化を追いづらくなります。",
    });
    base.push({
      action: "判別できない市販薬を多く併用する",
      reason: "負担が重なると、体調の要因分けが難しくなることがあります。",
    });
  } else if (topic === "疲労") {
    base.push({
      action: "寝る時間をさらに削る",
      reason: "回復の足がかりが減ると、改善の見極めがしづらくなります。",
    });
    base.push({
      action: "無理に激しい運動を取り入れる",
      reason: "急な負荷は、体調の波を読みづらくすることがあります。",
    });
  } else {
    base.push({
    ...
    base.push({
      action: "体調のメモを取らずに我慢を続ける",
      reason: "推移が分かりづらいと、次の判断のタイミングを逃しやすいです。",
    });
  }
...
async function buildConcreteImmediateActionsDetails(state, actionSection = "") {
  ...
  const [refinedActions, refinedDont, singaporeRestLines] = await Promise.all([
    refineDoActionsWithLLM(plan, state, state?.decisionLevel || "🟢", { forSummary: false }),
    refineDontActionsWithLLM(plan, state, seedDont, actionSection),
  ...
  const dontActions =
    Array.isArray(refinedDont) && refinedDont.length > 0 ? refinedDont : seedDont;
  return {
...
function mapDailyImpactAnswerToRestLevel(answer) {
  ...
  if (normalized === "ほぼ問題ない") return "NONE";
  if (normalized === "少しつらい") return "LIGHT";
  if (/^かなりつらい/.test(normalized)) return "STRONG";
...
function enforceSummaryIntroTemplate(text) {
  ...
  const templateBody = buildSummaryIntroTemplate();
  ...
  lines.splice(bodyStart, bodyEnd - bodyStart, ...templateLines);
  return lines.join("\n");
}
...
function getPrepareSeedsByCategory(category) {
  const seeds = {
    ...
    TIRED: [
      ["いつ頃からつらいか一言で言えるようにしておく", "診察がスムーズになります"],
      ["睡眠や仕事の負荷をメモしておく", "医師に伝えやすくなります"],
    ],
  };
  return seeds[category] || seeds.PAIN;
}
...
function getSlotOrderWithConditional(state) {
  const categoryEarly = state?.triageCategory || resolveQuestionCategoryFromState(state) || "PAIN";
  if (categoryEarly === "TIRED") {
    return ["duration", "daily_impact", "worsening_trend", "associated_symptoms", "cause_category"];
  }
  ...
  // triage 未保存でも resolve 済みのカテゴリを使う（SKIN/INFECTION 等が PAIN 扱いにならないよう categoryEarly と一致させる）
  const category = categoryEarly;
  if (category === "SKIN") {
  ...
  if (category === "SKIN") {
    // 見た目を痛みの直後（KAIRO_SPEC 7.1.1 / 14.0.0）。FIXED 列の worsening より前に来る
    const skinOrder = [
      "pain_score",
      "daily_impact",
      "worsening",
    ...
    base = skinOrder.filter((k) => base.includes(k));
  }
  return base;
}
...
function shouldDeprioritizeWorseningTrendSlot(state) {
  ...
  if (cat === "TIRED") {
    const d = String(state?.slotAnswers?.daily_impact || "").trim();
    if (/ほぼ問題|変わらない|大きく変わら|前と同じ|同程度|安定/.test(d)) {
      return true;
    }
  }
  return false;
...
/**
 * 未充填スロットを `getSlotOrderWithConditional` の**定義順**に並べる（KAIRO_SPEC 14.0 番号通り。スコア並べ替えは行わない）。
 * 従来の getSlotAskPriorityScore による並べ替えは、PAIN 等で duration より 3.5 やきっかけを先に出してしまうため廃止。
 * 「変わらない」系答済のときは 3.5 `worsening_trend` だけ最下位に回す。
 */
function sortMissingSlotsByPriority(missing, state) {
  ...
  let sorted = [...missing].sort((a, b) => orderIdx(a) - orderIdx(b));
  if (shouldDeprioritizeWorseningTrendSlot(state) && sorted.includes("worsening_trend")) {
    sorted = sorted.filter((k) => k !== "worsening_trend");
    sorted.push("worsening_trend");
  }
  return sorted;
}
...
function extractDurationFromText(text) {
  ...
  if (/(今日から|本日から|きょうから)/.test(normalized)) {
    return { raw_text: "今日から", normalized: "today", selectedIndex: 0 };
  }
...
function extractImpactFromText(text) {
  ...
  if (/普通に生活できる|普通に動ける|問題なく動ける|特に支障はない|日常生活は問題ない|ほぼ問題ない|ほとんど問題ない/.test(normalized)) {
    return {
      raw: pickRaw(
        /普通に生活できる|普通に動ける|問題なく動ける|特に支障はない|日常生活は問題ない|ほぼ問題ない|ほとんど問題ない/,
        rawText || normalized
      ),
      selectedIndex: 0,
    };
  }
  if (/かなりつらい|動くのがしんどい|動くのもしんどい/.test(normalized)) {
    return {
      raw: pickRaw(/かなりつらい|動くのがしんどい|動くのもしんどい/, rawText || normalized),
      selectedIndex: 2,
    };
  }
  if (/^少しつらい$|少しつらいです|ややつらい/.test(normalized)) {
    return { raw: pickRaw(/少しつらい|ややつらい/, rawText || normalized), selectedIndex: 1 };
  }
  return null;
}
...
function extractWorseningTrendFromText(text) {
  ...
  if (/(改善してきた|落ち着いてきた|回復に向か|回復してきている|回復してきた|良くなって|ましになって|楽になって|改善して)/.test(
    normalized
  )) {
    const m = rawText.match(
      /(改善してきた|落ち着いてきた|回復に向か[^。！？]*|回復してきている[^。！？]*|回復してきた[^。！？]*|良くなって[^。！？]*|ましになって[^。！？]*|楽になって[^。！？]*|改善して[^。！？]*)/
    );
...
function ensureSlotFilledConsistency(state) {
  ...
  for (const slotKey of SLOT_KEYS) {
    if (
      slotKey === "worsening_trend" &&
      !isDurationNotJustNow(state) &&
      (state?.triageCategory || resolveQuestionCategoryFromState(state)) !== "TIRED"
    ) {
      state.slotFilled[slotKey] = false;
...
/**
 * 喉が痛い系を INFECTION トリアージに乗せる（`isThroatMainSymptom` より広く、「喉に違和感」・嚥下痛・上気道/扁桃 等を含む）
 */
function isThroatInfectionTriageText(text) {
  if (!text || typeof text !== "string") return false;
  const n = String(text).replace(/\s+/g, "");
  if (!n) return false;
  if (
    /(喉|のど)(が|の|は|に|も)?(痛|違和|いたい|痛い|痛む|刺す|炎症|腫れ|腫れた|赤い|乾燥|カラカラ|イガイガ|しみる|かれ)/.test(n) ||
    /(咽頭痛|咽頭|扁桃|上気道|のどの痛み|喉の痛み|のどの違和感|喉の違和感|喉がひりひり|のどがひりひり|喉がゴロゴロ|のどがゴロゴロ)/.test(n)
  ) {
    return true;
  }
  if (/(飲み込|嚥下).{0,12}(痛|困難|しにく|つかえ|違和|でき)|嚥下痛/.test(n)) return true;
  if (/(喉|のど)(に|が|のは|は).{0,8}(痛|違和|不快感|しゃべ|声|いが|イガ|腫|赤|熱|かす)/.test(n)) return true;
  if (/(喉|のど).{0,5}(痛|腫|赤|いが|刺す|かす|壊死|違和|しゃべり|声が出|焼け)/.test(n)) return true;
  if (/(ノド|のーど|nodo|throat|sorethroat|sore[\s-]*throat|pharyng|tonsill)/i.test(n)) return true;
  return false;
}

/** 喉・のどが主症状かどうか（喉が痛い、のどの違和感など） */
function isThroatMainSymptom(text) {
  return isThroatInfectionTriageText(text);
}
...
function detectQuestionCategory4(text) {
  ...
  if (isThroatInfectionTriageText(normalized)) return "INFECTION";
  const infectionCue = /(熱|発熱|咳|せき|喉|のど|寒気|風邪|鼻水|鼻づまり)/.test(normalized);
  // だるい: INFECTION 手がかり（風邪・微熱等）があるときは後段 INFECTION へ。だる[さくんが] は活用形（だるくて・だるさ等）で「だるい」自体は含まない
  const tiredDirect =
    /(肩こり|首こり|首がこる|疲れ|疲労|だる[さくんが]|倦怠|睡眠不足|眠れない|寝不足|眠気|体が重い|慢性的|脱力|ぐったり|無気力)/.test(
      normalized
    ) || (/(だるい|倦怠)/.test(normalized) && !infectionCue);
  if (tiredDirect) {
    return "TIRED";
  }
...
function resolveLockedQuestionCategory(state, historyText = "") {
  const throatTriageText = [
    ...
    .filter(Boolean)
    .join(" ");
  if (isThroatInfectionTriageText(throatTriageText)) {
    state.triageCategory = "INFECTION";
    return "INFECTION";
  }
  ...
  if (fromState === "INFECTION" && state?.slotFilled?.associated_symptoms) {
    ...
    const hasThroatInPrior = isThroatInfectionTriageText(
      [
        state?.primarySymptom || "",
        state?.slotAnswers?.pain_score || "",
        state?.slotAnswers?.duration || "",
...
function getCategoryQuestionOverride(category, slotKey) {
  ...
  if (category === "TIRED") {
    if (slotKey === "duration") {
      return {
        question: "どのくらい続いていますか？",
        options: ["今日から", "数日前", "一週間以上前"],
      };
    }
    if (slotKey === "daily_impact") {
      return {
        question: "日常生活にどれくらい影響がありますか？",
        options: ["ほぼ問題ない", "少しつらい", "かなりつらい（動くのがしんどい）"],
      };
    }
    if (slotKey === "worsening_trend") {
      return {
        question: "今の状態はどうですか？",
        options: ["回復してきている", "変わらない", "悪化している"],
      };
    }
    if (slotKey === "associated_symptoms") {
      return {
        question: "他に気になる症状はありますか？",
        options: ["吐き気", "息苦しさ", "発熱がある"],
      };
    }
    if (slotKey === "cause_category") {
      return {
        question: "思い当たるきっかけはありますか？",
        options: ["寝不足・疲れ", "ストレス", "体を動かしすぎた"],
      };
    }
  }
  return null;
...
function buildIntroTemplateIds(state, questionIndex, slotKey) {
  const firstInOrder = (state && getSlotOrderWithConditional(state)[0]) || "pain_score";
  if (questionIndex === 0 && slotKey === firstInOrder) {
    return [];
  }
...
/** まとめの「Kairoの判断」ブロック内で、確認文用の前後文（今の情報から見ると、／という状況です。）を除去し箇条書きのみにする */
...
function sanitizeBulletPoints(bullets, state) {
  ...
  const cleaned = bullets
    ...
    .filter(Boolean);
  const thermalShort = dedupeBulletsKeepShortestInGroup(cleaned, isBodyThermalFeelingBulletLine);
  const giShort = dedupeBulletsKeepShortestInGroup(thermalShort, isGiLooseStoolBulletLine);
...
function inferBulletSlotKeyForDedupe(line) {
  ...
  if (isGiLooseStoolBulletLine(s)) {
    return `${BULLET_INFER_SLOT_KEYS.CONTEXT}:gi_loose_stool`;
  }
...
function hasMinConsecutiveRunOverlap(a, b, minRun = 2) {
  ...
  for (let k = 0; k <= na.length - minRun; k++) {
    if (nb.includes(na.slice(k, k + minRun))) return true;
  }
  for (let k = 0; k <= nb.length - minRun; k++) {
    if (na.includes(nb.slice(k, k + minRun))) return true;
  }
  return false;
}
...
function dedupeLinesKeepShorterOnOverlap(rawLines, getNorm) {
  if (!Array.isArray(rawLines) || rawLines.length <= 1) return rawLines || [];
  const normfn =
    getNorm || ((s) => normalizeBulletKeyForDedupe(String(s || "").replace(/^・\s*/, "")));
  const lines = rawLines.map((b) => String(b || "").trim()).filter(Boolean);
  const toRemove = new Set();
  for (let i = 0; i < lines.length; i++) {
    for (let j = i + 1; j < lines.length; j++) {
      if (toRemove.has(i) || toRemove.has(j)) continue;
      const lineI = lines[i].replace(/^・\s*/, "").trim();
      const lineJ = lines[j].replace(/^・\s*/, "").trim();
      const ni = normfn(lines[i]);
      const nj = normfn(lines[j]);
      if (ni.length < 2 && nj.length < 2) continue;
      const full = (A, B) => A.length > B.length && A.includes(B) && B.length >= 2;
      ...
      if (lineI.length < lineJ.length) toRemove.add(j);
      else if (lineJ.length < lineI.length) toRemove.add(i);
      else toRemove.add(j);
    }
  }
...
function dedupeBulletsBySubsumption(bullets) {
  ...
  const uniq = [];
  for (const b of [...new Set(bullets.map((x) => String(x || "").trim()).filter(Boolean))]) {
    const n = normalizeBulletKeyForDedupe(b);
    if (!n || seenNorm.has(n)) continue;
    seenNorm.add(n);
    uniq.push(b);
  }
  return dedupeLinesKeepShorterOnOverlap(uniq, (s) => normalizeBulletKeyForDedupe(s));
}

/**
 * 「冷え」と「冷えの可能性」、「お腹が痛くて」と「お腹が痛くて 4日間も続いている」のように、
 * 短い行が長い行の先頭と一致し長い行に追加情報があるときは**長い行を削除**（同趣旨重複を短い行に圧縮。KAIRO_SPEC と `dedupeLinesKeepShorterOnOverlap` 整合）。
 */
function dedupeBulletsShorterPrefixWhenLongerCompletesThought(bullets) {
  ...
  for (let i = 0; i < lines.length; i++) {
    for (let j = 0; j < lines.length; j++) {
      ...
      const [shorter, longer, longIdx] =
        ai.length < aj.length ? [ai, aj, j] : aj.length < ai.length ? [aj, ai, i] : [null, null, -1];
      if (!shorter || !longer || longIdx < 0) continue;
...
function isBodyThermalFeelingBulletLine(inner) {
  const s = String(inner || "").trim();
  if (!s) return false;
  if (/^痛み|痛みの|頭痛|歯痛|腹痛/.test(s)) return false;
  return /(体温|体が.*熱い|身体が.*熱い|熱っぽい|体が熱い|熱いことが|熱さ|寒気|微熱|平熱|発熱|熱が)/.test(s);
}

/** 下痢・軟便系 */
function isGiLooseStoolBulletLine(inner) {
  const s = String(inner || "").trim();
  return /(下痢|軟便|便がゆる|便の回数|お腹をゆる|腹がゆる|ゆるい便|下痢っぽ)/.test(s);
}

/**
 * 同一トピック（例：体感熱・下痢）が複数行に分かれているとき、**短い行を1行だけ残す**。
 */
function dedupeBulletsKeepShortestInGroup(bullets, testFn) {
  if (!Array.isArray(bullets) || bullets.length <= 1) return bullets || [];
  const idxs = [];
  for (let i = 0; i < bullets.length; i++) {
    const inner = String(bullets[i] || "").replace(/^・\s*/, "").trim();
    if (testFn(inner)) idxs.push(i);
  }
  if (idxs.length <= 1) return bullets;
  let bestIdx = idxs[0];
  let bestLen = String(bullets[bestIdx] || "").replace(/^・\s*/, "").trim().length;
  for (const i of idxs) {
    const L = String(bullets[i] || "").replace(/^・\s*/, "").trim().length;
    if (L < bestLen) {
      bestLen = L;
      bestIdx = i;
    }
  }
  const skip = new Set(idxs.filter((i) => i !== bestIdx));
  return bullets.filter((_, i) => !skip.has(i));
}
...
function dedupeStateAboutBulletsBySoftInclusion(bullets) {
  ...
  const lines = bullets.map((b) => String(b || "").trim()).filter(Boolean);
  const soften = (inner) => {
    let x = normalizeBulletKeyForDedupe(inner);
    return x.replace(/異常に|非常に|かなり|わりと|あまり|のことが多い|ことが多い|ことが|の可能性の可能性/g, "");
  };
  const polarityBucket = (inner) => {
    const s = String(inner || "");
    if (/ない|なし|見られない|出ていない|しません|しなかった/.test(s)) return "neg";
    if (/ある|続い|見られる|出ている|多い|続く|始ま/.test(s)) return "pos";
    return "neu";
  };
  const minCore = 6;
  const removeIdx = new Set();
  for (let i = 0; i < lines.length; i++) {
    for (let j = 0; j < lines.length; j++) {
      ...
      const pi = polarityBucket(ai);
      const pj = polarityBucket(aj);
      if (pi !== pj && pi !== "neu" && pj !== "neu") continue;
      const si = soften(ai);
      const sj = soften(aj);
      if (si.length < minCore || sj.length < minCore) continue;
      if (si.length < sj.length && sj.includes(si)) {
        removeIdx.add(j);
...
function dedupeBulletsByTwoCharRunOverlapRespectingPolarity(bullets) {
  ...
  const polBucket = (inner) => {
    const s = String(inner || "");
    if (/ない|なし|見られない|出ていない|しません|しなかった|ないです/.test(s) && !/(可能性|ことが)/.test(s)) {
      return "neg";
    }
...
/** きっかけスロットの生回答（整形前）を取得。空・不明は除外。 */
function getCauseRawSlotsAnswer(state) {
  const raw = String(
    state?.causeDetailText ||
...
function comboCauseBulletInnerToShortLabel(rawLine) {
  const inner = stripBulletLead(String(rawLine || ""))
    .replace(/^きっかけ[・：:]\s*/, "")
    .replace(/^きっかけは/, "")
    .trim();
  if (!inner) return "";
...
/** 🟢🟡①：きっかけを 3〜7 文字目安の名詞句に（同期・ヒューリスティック） */
function buildCauseComboShortLabelSync(state) {
  if (!state || !comboSlotAllowsGreenYellowCombo(state, "cause_category")) return "";
  const raw = getCauseRawSlotsAnswer(state);
  if (!raw) return "";
...
/** 🟢🟡①：きっかけ短語。API があるときは LLM で 3〜7 文字に整形し、失敗時は `buildCauseComboShortLabelSync` */
async function buildCauseComboShortLabelAsync(state) {
  const sync = buildCauseComboShortLabelSync(state);
  if (!sync) return "";
  if (!process.env.OPENAI_API_KEY) return sync;
  const raw = getCauseRawSlotsAnswer(state);
  if (!raw) return sync;
  const written = polishCausePhraseToWrittenJapanese(raw);
  if (!written) return sync;
  const llm = await shortenComboLabelsForCombinationLineWithLlm(state, [written]);
  if (llm && llm[0]) {
    const t = finalizeComboLabelForCombinationLine(llm[0]);
    if (t) return t;
  }
  return sync;
}

/** 🔴組み合わせ行：きっかけ HIGH 時の短語（ユーザー語の核＋3〜7文字目安） */
function buildRedCauseComboShortLabelForState(state) {
  const cr = String(state?.causeDetailText || state?.slotAnswers?.cause_category || "").trim();
...
/**
 * 🟢🟡「① 状態の定義」：組み合わせ短語は**まとめ箇条書き**由来のみ。スロット生値直列・`extractFeatures(履歴+…)` は使わない（KAIRO_SPEC・🔴と同方針）。
 */
function collectGreenYellowLowMediumCombinationParts(state) {
  ...
  const pushC = (c) => {
    const lab = String(c?.label || "").trim();
    if (!lab || isMainLabel(lab) || seenLab.has(lab)) return;
    seenLab.add(lab);
    candidates.push(c);
  };
  for (const c of [
    ...gatherGreenYellowComboCandidatesFromBullets(state),
    ...supplementGreenYellowComboCandidatesFromAllBulletLines(state),
  ]) {
    pushC(c);
  }
  ...
  if (Array.isArray(raw) && raw.length >= 1) {
    const hasPainHead = raw[0] && /^痛みは\s*\d+\s*\/\s*10/.test(stripBulletLead(raw[0]));
    const mainIdx = hasPainHead ? 1 : 0;
    const directCands = [];
    for (let i = 0; i < raw.length; i++) {
      if (i === mainIdx) continue;
      const t =
        shortenComboLabelFromBulletText(stripBulletLead(raw[i])) ||
        finalizeComboLabelForCombinationLine(stripBulletLead(raw[i]));
      if (t && !isMainLabel(t) && !directCands.some((d) => d.label === t)) {
        directCands.push({ label: t, inv: 60 - i });
      }
    }
    picked = finish(
      pickGreenYellowComboPartsReservingPainManner(
        applyRedGuardComboFilter(directCands),
        state,
        2,
        comboMaxParts
      )
...
/**
 * きっかけを書き言葉に整えた**長文**（LLM 入力・デバッグ用）。組み合わせ「〇〇」列挙には `buildCauseComboShortLabelSync`／`buildCauseComboShortLabelAsync` を使う。
 */
function getPolishedCauseForGreenYellowCombo(state) {
  if (!state || !comboSlotAllowsGreenYellowCombo(state, "cause_category")) return "";
  const raw = getCauseRawSlotsAnswer(state);
  if (!raw) return "";
  const w = polishCausePhraseToWrittenJapanese(raw);
  if (!w) return "";
...
function dedupeComboLabelsBySubsumption(labels) {
  ...
  return dedupeLinesKeepShorterOnOverlap(lines, (s) => normalizeBulletKeyForDedupe(s));
}
 ...
 * **合計チャンク数は最大 3**（きっかけ＋短語で 4 連になるのを防ぐ）。箇条書きとの重複で短語を落とすと combo が空になりやすいため、列挙は dedupe のみ行う（KAIRO_SPEC・まとめ①の箇条書きと🤝本文は別枠）。
 * @param {string} [causeShortOverride] - 非同期経路で LLM 済みのきっかけ短語。未指定時は同期ヒューリスティック。
 */
function buildGreenYellowComboPlusClause(state, parts, causeShortOverride) {
  const MAX_CHUNKS_IN_COMBO_CLAUSE = 3;
  let labels = (parts || []).map((p) => String(p).trim()).filter(Boolean);
  let causeW = "";
  if (causeShortOverride !== undefined && causeShortOverride !== null) {
    causeW = String(causeShortOverride).trim();
    causeW = finalizeComboLabelForCombinationLine(causeW) || causeW;
  } else if (state) {
  ...
  labels = dedupeComboLabelsBySubsumption(labels);
  {
    ...
    for (const l of labels) {
      ...
      for (let k = 0; k < pair.length; k++) {
        if (!comboLabelsAreDuplicateForCombo(pair[k], t)) continue;
        if (t.length < pair[k].length) pair[k] = t;
        skip = true;
        break;
      }
      if (skip) continue;
  ...
  const merged = dedupeLinesKeepShorterOnOverlap(
    chunks,
    (s) => normalizeBulletKeyForDedupe(String(s || ""))
  ).slice(0, MAX_CHUNKS_IN_COMBO_CLAUSE);
...
・箇条書きの**すべて**を盛り込む必要はない。安心・納得に効く**要点だけ**選び、自然な1〜2文にまとめる。
...
function pickGreenYellowComboPartsReservingPainManner(candidates, state, min, max) {
  ...
  const pushIfNew = (lab) => {
    ...
    for (let k = 0; k < uniq.length; k++) {
      const u = uniq[k];
      if (!comboLabelsAreDuplicateForCombo(u, t)) continue;
      if (t.length < u.length) uniq[k] = t;
      return false;
    }
    uniq.push(t);
    return true;
...
/**
 * まとめ箇条書きから主症状行（痛み/10 直後等）を除き、各行を組み合わせ短語化（🔴/🟢/🟡 共通。確認文・生履歴は使わない）。
 */
function enumerateComboShortLabelsFromNonMainBullets(state) {
  const raw = state?.stateAboutBulletsCache;
  if (!Array.isArray(raw) || raw.length < 1) return [];
  const main = compactMainSymptomNounForRed(state);
  const hasPainHead = raw[0] && /^痛みは\s*\d+\s*\/\s*10/.test(stripBulletLead(raw[0]));
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (main && main !== "症状" && rawLine === main) continue;
    const short =
      shortenComboLabelFromBulletText(rawLine) || finalizeComboLabelForCombinationLine(rawLine);
    if (!short) continue;
    out.push({ label: short, inv: 50 - i });
  }
  return out;
}

/**
 * まとめ箇条書き（Kairoの判断の・行と同一キャッシュ）から〇〇用の短語候補を得る。🔴 用（redComboTier 付与）。
 */
function supplementRedComboCandidatesFromAllBulletLines(state) {
  return enumerateComboShortLabelsFromNonMainBullets(state).map((c) => ({
    ...c,
    redComboTier: 1,
  }));
}

/** 🟢🟡 用：上と同内容で `{ label, inv }` のみ。 */
function supplementGreenYellowComboCandidatesFromAllBulletLines(state) {
  return enumerateComboShortLabelsFromNonMainBullets(state);
}

/**
 * 🔴 組み合わせ行「〇〇＋〇〇」：KAIRO_SPEC — 用語はまとめ箇条書き由来のみ（推定・確認文抜粋は使わない）。
 */
function collectRedHighRiskCombinationLabels(state) {
  ...
  for (const c of [
    ...
    ...supplementRedComboCandidatesFromAllBulletLines(state),
  ]) {
    const lab = String(c?.label || "").trim();
    if (!lab || isMainLabel(lab) || seenLab.has(lab)) continue;
    seenLab.add(lab);
    candidates.push(c);
  }
  ...
  if (Array.isArray(raw) && raw.length >= 1) {
    ...
    for (let i = 0; i < raw.length; i++) {
      ...
      const t =
        shortenComboLabelFromBulletText(stripBulletLead(raw[i])) ||
        finalizeComboLabelForCombinationLine(stripBulletLead(raw[i]));
      if (t && !isMainLabel(t) && !direct.includes(t)) direct.push(t);
    }
    picked = applyRedComboUserWordGuards(state, direct.slice(0, 3));
  }
  if (picked.length >= 2) return picked;
  return applyRedComboUserWordGuards(state, ["つらさが強い", "注意が必要な兆候"]);
}
...
/** まとめ「🤝/📝 Kairoの判断」箇条書き直後（🟢🟡は KAIRO_SPEC 3 ブロック固定・🔴は別構成・🔴意味行は async 版を使用） */
...
/**
 * 原因モーダル LLM 用：箇条書き・スロットから事実が1行も取れないときの最小フォールバック（userSummary 空文字を防ぐ）。
 */
function buildMinimalModalUserSummaryFallback(state) {
  if (!state) return "";
  const parts = [];
  const label = String(getSyncedMainSymptomDisplayLabel(state) || "").trim();
  if (label) parts.push(`主症状（表示ラベル）：${label}`);
  const cat = state.triageCategory || resolveQuestionCategoryFromState(state);
  if (cat) parts.push(`トリアージカテゴリ：${cat}`);
  const firstLine = String(state.historyTextForCare || "")
    .split("\n")
    .map((l) => l.trim())
    .find((l) => l.length > 0);
  if (firstLine && firstLine.length > 2) {
    parts.push(`会話の冒頭：${firstLine.slice(0, 200)}`);
  }
  if (parts.length > 0) return parts.join("\n");
  return "（ユーザー情報の詳細は未取得。主症状・緊急度・検索スニペットを参照。）";
}

/** モーダル用ユーザー事実の1回分収集（リトライループから呼ぶ） */
function collectModalUserFactsAndSummaryOneAttempt(state, summaryFacts = [], summarySection = "") {
  const bulletLines = extractBulletLinesFromText(summarySection || "");
  const fromCache = Array.isArray(state?.stateAboutBulletsCache)
    ? state.stateAboutBulletsCache.map((line) => toBulletText(String(line || ""))).filter(Boolean)
    : [];
  ...
  if (state) {
    const built = buildStateFactsBullets(state, { forSummary: true });
    const inners = (Array.isArray(built) ? built : [])
  ...
  for (const entry of candidates) {
    const l = String(entry || "")
      .replace(/^[・\s]+/, "")
      .replace(/^→\s*/, "")
      .trim();
    if (!l || l.length > 80) continue;
    if (isPainStrengthSummaryLineForModal(l)) continue;
    if (seen.has(l)) continue;
    seen.add(l);
    rawFacts.push(l);
  }

  const userSummary = rawFacts.slice(0, 12).join("\n");
  return { rawFacts, userSummary };
}

/**
 * 「Kairoの判断」由来の事実配列と結合テキスト（🤝/📝モーダル用）。
 * 1回目で空なら会話同期のうえ**最大2回**再試行し、それでも空なら最小フォールバック。
 */
function collectModalUserFactsAndSummary(state, summaryFacts = [], summarySection = "") {
  const MAX_ATTEMPTS = 3;
  let last = { rawFacts: [], userSummary: "" };
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    if (state && attempt > 0) {
      syncHistoryTextForCareFromConversation(state);
    }
    last = collectModalUserFactsAndSummaryOneAttempt(state, summaryFacts, summarySection);
    if (last.userSummary.trim()) {
      return last;
    }
    if (!state) break;
  }
  if (!last.userSummary.trim() && state) {
    const fb = buildMinimalModalUserSummaryFallback(state);
    return { rawFacts: [fb], userSummary: fb };
  }
  return last;
}
...
const YELLOW_CAUTION_FACT_LABELS = {
  pain_score: "痛みの強さ",
  worsening: "痛み方・質",
  duration: "経過・いつから",
  daily_impact: "日常生活への影響",
  associated_symptoms: "付随症状",
  worsening_trend: "経過の方向",
  cause_category: "きっかけ・原因",
};
...
function buildYellowCautionMediumHighFactsForPrompt(state) {
  if (!state) return "";
  const val = (statusKey, answerKey) =>
    getSlotStatusValue(state, statusKey, state?.slotAnswers?.[answerKey] || "");
  const lines = [];
  const pushIf = (slotId, statusKey, answerKey, rl) => {
    if (!slotRiskIsMediumOrHighForYellowCaution(rl)) return;
    const text = String(val(statusKey, answerKey) || "").trim();
    if (text.length < 2) return;
    const label = YELLOW_CAUTION_FACT_LABELS[slotId] || slotId;
    lines.push(`・${label}：${text.slice(0, 160)}`);
  };
  pushIf("pain_score", "severity", "pain_score", getPainScoreRiskLevelForYellowCaution(state));
  pushIf("worsening", "worsening", "worsening", state?.slotNormalized?.worsening?.riskLevel);
  pushIf("duration", "duration", "duration", state?.slotNormalized?.duration?.riskLevel);
  pushIf("daily_impact", "impact", "daily_impact", state?.slotNormalized?.daily_impact?.riskLevel);
  pushIf(
    "associated_symptoms",
    "associated",
    "associated_symptoms",
    state?.slotNormalized?.associated_symptoms?.riskLevel
  );
  pushIf(
    "worsening_trend",
    "worsening_trend",
    "worsening_trend",
    state?.slotNormalized?.worsening_trend?.riskLevel
  );
  pushIf("cause_category", "cause_category", "cause_category", state?.slotNormalized?.cause_category?.riskLevel);
  return lines.join("\n").trim();
}

function resolveYellowCautionUserContentForPrompt(state, userSummary) {
  const mh = buildYellowCautionMediumHighFactsForPrompt(state);
  const us = String(userSummary || "").trim();
  if (mh.trim() && us) {
    return `【緊急度の内訳（中リスク・高リスクの要因のみ。低リスク相当は含めない）】\n${mh}\n\n【その他・まとめ箇条書き・スロット参照】\n${us}`;
  }
  if (mh.trim()) {
    return `【緊急度の内訳（中リスク・高リスクの要因のみ。低リスク相当は含めない）】\n${mh}`;
  }
...
/** 原因モーダル「→」右で避けたい定型（〇〇の炎症が△△を引き起こす… 等） */
function looksLikeModalInflammationCauseArbitraryTemplate(tail) {
  const s = String(tail || "").trim();
  if (!s) return false;
  if (/の炎症が[^。]{0,24}(を引き起こす|を生じさせる)/.test(s)) return true;
  if (/炎症が[^。]{0,20}を引き起こす/.test(s)) return true;
  if (/を引き起こすことがある[。.]?$/.test(s) && /炎症/.test(s)) return true;
  return false;
}

/** 「→」右がユーザー訴えの言い換えに偏っているとき */
function looksLikeUserStateParaphraseModalReason(tail) {
  return /感じている|感じます|優れないと|つらさを(訴え|感じ)|おっしゃ|言ってい|主観的|今のあなた|あなたの体調/.test(String(tail || ""));
}

function modalReasonTailReferencesMainSymptom(tail, mainSymptomDisplay) {
  const m = String(mainSymptomDisplay || "").trim();
  if (m.length < 2) return false;
  return String(tail || "").includes(m);
}
...
/**
 * 「→」右が左と対応しない／定型に落ちたときのフォールバック。
 * 旧テンプレ（「…説明されうる病態・機序として整理…」系）はモーダルに出さない（`MODAL_REASON_FALLBACK_GENERIC` のみ使用）。
 */
function buildModalCauseAlignedReasonTail(_causeHead, _mainSymptomDisplay) {
...
/**
 * 原因モーダル「→」右：**一文のみ**・句点「。」は**文末に1つ**（文中に「。」を複数置かない）。
 * 複数文が返った場合は**先頭の1文**に切り詰める。
 */
function normalizeModalReasonTailToSingleSentence(tail) {
  let t = String(tail || "").replace(/\s+/g, " ").trim();
  if (!t) return t;
  t = t.replace(/．/g, "。");
  t = t.replace(/^。+/g, "").trim();
  if (!t) return "";
  ...
  const dup = t.indexOf("。", 1);
  if (dup > 0) t = t.slice(0, dup + 1);
  return t.trim();
}

/** 「→」右の理由を最終サニタイズ（痛みスロット禁止・ノイズ除去・一文化・短すぎるときはフォールバック） */
function sanitizeModalArrowReasonTail(raw) {
  ...
  t = normalizeModalReasonTailToSingleSentence(t);
  t = t.replace(/\s{2,}/g, " ").trim();
  if (
    !t ||
    t.length < 5 ||
  ...
  // LLM や旧実装が混入しても出さない：教科書調の「病態・機序として整理」系
  if (
    /説明されうる病態|機序として整理|症状の一つとして説明されうる|病態・機序として整理/.test(t)
  ) {
    return MODAL_REASON_FALLBACK_GENERIC;
  }
  return t;
...
function finalizeModalCauseDisplayLine(item, mainSymptomDisplayForAlign) {
  ...
  const alignLabel =
    mainSymptomDisplayForAlign != null && String(mainSymptomDisplayForAlign).trim() !== ""
      ...
      : "";
  if (alignLabel) {
    const stiff = !tail || tail === MODAL_REASON_FALLBACK_GENERIC;
    const badTmpl = looksLikeModalInflammationCauseArbitraryTemplate(tail);
    const userOnly = looksLikeUserStateParaphraseModalReason(tail);
    const linked = modalArrowReasonReferencesCause(head, tail);
    const longNatural =
      String(tail).length >= 22 &&
      modalReasonTailReferencesMainSymptom(tail, alignLabel) &&
      !userOnly &&
      !badTmpl;
    if (stiff || badTmpl || userOnly) {
      tail = sanitizeModalArrowReasonTail(buildModalCauseAlignedReasonTail(head, alignLabel));
    } else if (!linked && !longNatural) {
      tail = sanitizeModalArrowReasonTail(buildModalCauseAlignedReasonTail(head, alignLabel));
    }
  }
  return `・${head} → ${tail}`;
...
async function buildDiseaseSafetyFilteredMessage(
  ...
  const tryExtract = async (temperature = 0.2, whyRetryAttempt = 0) => {
    const prompt = [
      ...
      "- **「→」の右（理由）**：**必ず一文**で終える。**句点「。」は文末に1つだけ**（文中に「。」を複数置かない。2文に分けない）。直前の「→」左の原因（疾患・機序）が**どんな病態・症状として扱われるか**を簡潔に述べ、**主症状（表示ラベル）**と鑑別上どう対応づけられるかを含める。左の原因名（またはその略称・語）に**必ず対応**させる。左と無関係な一般論・別の病名の説明だけは禁止。**同じ理由文を複数行に流用しない**。**禁止**：「○○の炎症が△△を引き起こすことがある」型の定型句や、炎症語と主症状を機械的に結ぶ短文の連発。「引き起こす」の多用も避ける。検索結果に沿う。ユーザー発言の言い換え・心理描写・経過の要約は書かない。",
      ...
      "- conditional = 条件付きで考慮すべき状況（**ちょうど2件**）。各項目は「・<病名> → <関連した理由>」形式。**主症状に関連する病名のみ**使用。主症状と無関係な病名は絶対に含めない。**「→」右は必ず左の病名・原因に対応した説明で一文**（句点「。」は文末に1つだけ）。左と無関係な一般論のみは禁止。理由はユーザー症状と関連付ける。検索結果＋ユーザー症状から要約。煽らない表現。固定テンプレート禁止。**感染症系（ウイルス・細菌・上気道炎・胃腸炎など）はここに分類する**（common に出さない）。",
  ...
  // common が3件未満のとき、主症状に紐づいた fallback から補う（感染症系は入れない・例は常に十分出るようにする）
  if (common.length < 3) {
  ...
  if (common.length < 3) {
    ...
    const seen = new Set(common.map(causeKeyForRefill).filter(Boolean));
    for (const item of pool) {
      if (common.length >= 3) break;
      const name = (item.match(/^・([^→]+)/) || [])[1]?.trim?.() || "";
      const nameKey = normalizeModalDiseaseToken(name).toLowerCase().replace(/\s+/g, "");
  ...
  conditional = conditional.slice(0, 2);
...
function buildCurrentStateContext(state, historyText = "", concreteMessage = "") {
  ...
  // 🤝 Kairoの判断のクエリ（主症状表示＋カテゴリ＋箇条書き全文）。今すぐやることはこれに「対処法」を加える。
...
async function buildLocalSummaryFallback(level, history, state) {
  ...
  const aboutLineGy =
    (await buildStateAboutLineAsync(state, level).catch(() => "")) ||
    buildGreenYellowStateAboutBlock(state, level);
  const handshakeBodyGy = buildGreenYellowHandshakeBlockBodySync(
    state,
    String(aboutLineGy || "").trim() || "症状の状態を確認しました。"
  );
  const outlookBlockText =
    (await buildOutlookBlockWithLlm(state).catch(() => null)) || buildOutlookBlock(state);
  const baseBlocks = [
    `${level} ここまでの情報を整理します\n${buildSummaryIntroTemplate()}`,
    `🤝 Kairoの判断\n${handshakeBodyGy}`,
    immediateBlock,
...
function mapFreeTextToOptionIndex(answer, options, type) {
  ...
  if (options.length === 3) {
    ...
    // TIRED: 経過
    if (indexOf("回復してきている") >= 0) {
      if (/悪化している|悪化してきた|ひどくなって/.test(text)) return 2;
      if (/変わらない|横ばい|同じ|変化なし/.test(text)) return 1;
      if (/回復してき|回復に向か|良くなって|改善/.test(text)) return 0;
    }
    // TIRED: 危険サイン
    if (indexOf("息苦しさ") >= 0 && indexOf("発熱がある") >= 0) {
      if (/吐き気|嘔吐|むかむか/.test(text)) return 0;
      if (/息苦|呼吸困難|息が詰まる/.test(text)) return 1;
      if (/発熱|熱がある|熱っぽい|体温/.test(text)) return 2;
    }
    // TIRED: きっかけ
    if (indexOf("体を動かしすぎた") >= 0) {
      if (/寝不足|眠れ|疲れ|睡眠不足/.test(text)) return 0;
      if (/ストレス|緊張/.test(text)) return 1;
      if (/動かしすぎ|運動|仕事しすぎ/.test(text)) return 2;
    }
    // worsening_trend（3.5・全カテゴリ共通）
    if (indexOf("発症時より悪化している") >= 0) {
      if (/発症時より悪化|悪化している|ひどくなって|悪化してきた/.test(text)) return 2;
      if (/変わらない|横ばい|同じ|変化なし/.test(text)) return 1;
      if (/回復に向か|回復してき|良くなって|ましになって|楽になって|改善して/.test(text)) return 0;
    }
  }
  ...
  if (type === "daily_impact") {
    if (indexOf("ほぼ問題ない") >= 0) {
      if (/かなりつらい|動くのがしんどい|動けない|寝込/.test(text)) return 2;
      if (/少しつらい|ややつらい/.test(text)) return 1;
      if (/ほぼ問題|問題ない|大丈夫|普通/.test(text)) return 0;
    }
...
function calculateRiskFromState(state) {
  const triageCat = state?.triageCategory || resolveQuestionCategoryFromState(state);
  ...
  const scores =
    triageCat === "TIRED"
      ? {
          pain: 0,
          quality: mapRiskLevelToSeverityScore(state?.slotNormalized?.worsening_trend?.riskLevel),
...
async function generateSummaryForConfirmation(conversationId) {
  ...
  return { message: aiResponse || "", followUpQuestion, followUpMessage: null };
    ...
    const [lastBlock, actionsBlock, outlookBlockMinimal] = await Promise.all([
      (async () => {
        for (let i = 0; i < 5; i++) {
          try {
    ...
    let handshakeAbout =
      aboutLineGy && String(aboutLineGy).trim()
        ? aboutLineGy
        : buildGreenYellowStateAboutBlock(state, level);
    if (!String(handshakeAbout || "").trim()) {
      handshakeAbout = "症状の状態を確認しました。";
    }
    const handshakeBody = buildGreenYellowHandshakeBlockBodySync(state, handshakeAbout);
    const minimal = [
      `${level} ここまでの情報を整理します`,
      buildSummaryIntroTemplate(),
      "",
      ...
      "",
      lastBlock || "🌱 最後に\n今の情報なら安心して休んで大丈夫です。まずは休息を優先し、不安になったらまたここで確認してください。",
    ].join("\n");
...
app.post("/api/chat", async (req, res) => {
  ...
  let locationRePromptBeforeSummary = null;
    ...
    if (!state.triageCategory) {
      ...
      // 喉が痛い系は KAIRO_SPEC どおり INFECTION。上気道に寄らない曖昧な INFECTION 検出だけ PAIN に落とす
      if (isThroatInfectionTriageText(message)) {
        state.triageCategory = "INFECTION";
      } else if (detected === "INFECTION") {
        state.triageCategory = "PAIN";
      } else {
        state.triageCategory = detected;
      }
    }
    ...
    if (isWaitingForConfirmationResponse && !conversationState[conversationId].summaryShown) {
          ...
          let handshakeAbout =
            level === "🟢" || level === "🟡"
              ? (await buildStateAboutLineAsync(state, level).catch(() => "")) ||
                buildGreenYellowStateAboutBlock(state, level)
              : "";
          if ((level === "🟢" || level === "🟡") && !String(handshakeAbout || "").trim()) {
            handshakeAbout = "症状の状態を確認しました。";
          }
          const handshakeBlock =
            level === "🟢" || level === "🟡"
              ? buildGreenYellowHandshakeBlockBodySync(state, handshakeAbout)
              : stateBlock;
      ...
      return res.json({
        ...
        questionPayload: null,
        normalizedAnswer: state.lastNormalizedAnswer || null,
        followUpQuestion: shouldSendFollowUpQuestion(sections, summaryMsg)
          ? followUpQ || getInitialFollowUpQuestionBySpec(state)
          : null,
        followUpMessage: null,
        locationPromptMessage,
        ...
        return res.json({
          ...
          questionPayload: null,
          normalizedAnswer: state.lastNormalizedAnswer || null,
          followUpQuestion: shouldSendFollowUpQuestion(sections, fallbackSummary)
            ? getInitialFollowUpQuestionBySpec(state)
            : null,
          followUpMessage: null,
          locationPromptMessage,
          locationRePromptMessage: null,
...
`;
    ...
    if (followUpQuestion && shouldSendFollowUpQuestion(sections, aiResponse)) {
      conversationHistory[conversationId].push({
        role: "assistant",
    ...
    res.json({
      ...
      questionPayload,
      normalizedAnswer,
      followUpQuestion: shouldSendFollowUpQuestion(sections, aiResponse)
        ? followUpQuestion || null
        : null,
    ...
    if (state && (filled >= getRequiredSlotCount(state) || wasWaitingForSummary)) {
      ...
      let fallbackSummary = await enforceSummaryStructureStrict(
        localFallback,
        level,
        history,
        state
      ...
      if ((level === "🟢" || level === "🟡") && state) {
        const careForHandshake = (history || [])
          .filter((m) => m.role === "user")
          .map((m) => m.content)
          .join("\n");
        fallbackSummary = await ensureHandshakeStateBlockFull(fallbackSummary, state, careForHandshake);
      }
      const triage = {
        judgement: level,
        confidence: state.confidence || 0,
      ...
      return res.status(200).json({
        ...
        questionPayload: null,
        normalizedAnswer: state?.lastNormalizedAnswer || null,
        followUpQuestion: shouldSendFollowUpQuestion(sections, fallbackSummary) ? fallbackFq : null,
        followUpMessage: null,
        judgeMeta: {
          judgement: level,
          confidence: state.confidence || 0,
...
/**
 * 本文から「最後に出たまとめブロック見出し行」を取得（sections 抽出失敗時のフォロー可否判定用）。
 */
function findLastSummaryBlockHeaderLine(text) {
  const lines = String(text || "").split("\n");
  const headerRe = /^(🟢|🟡|🔴|🤝|✅|⏳|📝|🏥|💬|🌱)\s/;
  for (let i = lines.length - 1; i >= 0; i--) {
    const L = String(lines[i] || "").trimStart();
    if (!L) continue;
    if (headerRe.test(L)) return L;
    if (/^[\u{1F331}\u{1F4AC}]\s*最後に/u.test(L) || /^[🌱💬]\s*最後に/.test(L)) return L;
  }
  return "";
}

/**
 * フォロー文を送る条件: 最後のセクションが「最後に」（🌱 or 💬）で始まること。
 * sections が空／境界ずれでも、本文の最終ブロック見出しが「最後に」なら true（順序正規化後と整合）。
 */
function shouldSendFollowUpQuestion(sections, fullText) {
  if (Array.isArray(sections) && sections.length > 0) {
    const lastSection = sections[sections.length - 1];
    const normalized = String(lastSection || "").normalize("NFC").trim();
    const firstLine = normalized.split("\n")[0] || "";
    if (
      /^[\u{1F331}\u{1F4AC}]\s*最後に/u.test(firstLine) ||
      /^[🌱💬]\s*最後に/.test(firstLine)
    ) {
      return true;
    }
  }
  const lastHeader = findLastSummaryBlockHeaderLine(fullText);
  return (
    /^[\u{1F331}\u{1F4AC}]\s*最後に/u.test(lastHeader) || /^[🌱💬]\s*最後に/.test(lastHeader)
  );
}
