...
- 本文は最大3文。シンプルで短く。抽象的な励まし禁止（「大切に過ごしてください」等）。
...
async function ensureLastBlock(text, level, state = null, contextText = "") {
  ...
  const { found, body } = hasBlock(text);
  if (found && body && body.length >= 20) return text;
...
${rules}
本文は最大3文。シンプルで短く。行動につながる内容のみ。見出し行は出力しない。本文のみ。`;
  try {
    const completion = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      messages: [
    ...
    const body = (completion?.choices?.[0]?.message?.content || "").trim().replace(/^[🌱💬]\s*最後に\s*\n?/i, "");
    if (body && body.length > 10) return `${header}\n${body}`;
  } catch (_) {}
  const fallback =
    level === "🔴"
